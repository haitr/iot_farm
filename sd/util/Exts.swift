...
extension NSObject {
    @objc func performBlock(_ block: () -> Void) {
        block();
    }
    
    func performBlock(_ block: @escaping () -> Void, afterDelay delay: TimeInterval) {
        perform(#selector(self.performBlock(_:)), with: block, afterDelay: delay)
    }
}
...
extension String {
    // Subscript String with an integer range
